...
export function KatalogClient() {
  ...
  return (
    <>
            ...
            <div>
              ...
              <div className="relative">
                <input
                  ...
                  className="w-full rounded-lg border border-zinc-300 bg-white px-3 py-2.5 pr-10 text-sm text-zinc-900 placeholder-zinc-400 focus:outline-none focus:ring-2 focus:ring-amber-500"
                />
                <button
                  onClick={() => updateSearchParams({ search: searchInput })}
                  className="absolute right-1.5 top-1/2 -translate-y-1/2 p-1.5 text-zinc-400 hover:text-amber-700"
                >
                ...
                {categories.map((cat) => (
                  ...
                  >
                    <input
                      ...
                      className="h-4 w-4 text-amber-700 border-zinc-300 focus:ring-amber-500"
                    />
        ...
        <div className="flex-1 min-w-0">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
            ...
            <div className="flex items-center gap-3">
              ...
              <select
                ...
                className="rounded-lg border border-zinc-300 bg-white px-3 py-2 text-sm text-zinc-700 focus:outline-none focus:ring-2 focus:ring-amber-500"
              >
          ...
          {loading ? (
            ...
            <div className="text-center py-20">
              ...
              <button onClick={resetFilters} className="text-sm font-medium text-amber-700 hover:text-amber-800">
                Reset Filter
              </button>
            </div>
          ) : (
      ...
      <div className="lg:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-zinc-200 p-3 flex gap-2 z-40">
        <button
          onClick={() => {
            const params = new URLSearchParams(searchParams.toString())
            if (category_id) params.set("category_id", category_id)
        ...
        <button onClick={resetFilters} className="flex-1 text-center text-sm font-medium text-amber-700 border border-amber-300 rounded-lg py-2.5">
          Reset
        </button>
      </div>
    </>

...
export default function MetricCard({
  ...
  return (
    <motion.div
      ...
      className="group relative bg-white dark:bg-zinc-800/50 rounded-2xl border border-zinc-200 dark:border-zinc-700/50 p-6 sm:p-8 hover:shadow-xl hover:shadow-gold/5 transition-shadow duration-500"
    >
      ...
      <div className="relative">
        ...
        <div className="mt-4 text-3xl sm:text-4xl font-bold text-zinc-900 dark:text-white">
          <Counter from={0} to={value} inView={inView} />
          {suffix && <span className="text-gold">{suffix}</span>}
        </div>
        <h3 className="mt-2 font-semibold text-zinc-900 dark:text-white">{title}</h3>

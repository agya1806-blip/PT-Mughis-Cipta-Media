...
export default function QuoteCard({
  ...
  return (
    ...
    >
      ...
      <p className="text-green-dark/80 leading-relaxed">&ldquo;{quote}&rdquo;</p>
      <div className="mt-6 pt-4 border-t border-gold/10 flex items-center gap-3">
        {avatar ? (
          <div className="shrink-0">{avatar}</div>
        ) : (
          <div className="w-10 h-10 rounded-full bg-gold/10 flex items-center justify-center shrink-0">
            <span className="text-green-dark font-semibold text-sm">{author[0]}</span>
          </div>
        )}
        <div>
          <p className="font-semibold text-green-dark text-sm">{author}</p>
          {role && <p className="text-xs text-green-dark/80">{role}</p>}
        </div>
      </div>
    </motion.div>
  )

...
export function FeaturedBooksCarousel({ books }: Props) {
  ...
  const next = () => {
    setDirection(1)
    setCurrent((p) => (p + 1) % books.length)
  }

  const prev = () => {
    setDirection(-1)
    setCurrent((p) => (p - 1 + books.length) % books.length)
  }
        ...
        >
          <div>
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-gold/10 border border-gold/20 mb-6">
              <span className="text-gold text-xs font-medium uppercase tracking-wider">Katalog</span>
            </div>

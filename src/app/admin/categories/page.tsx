    ...
    <div>
      ...
      <form onSubmit={handleSubmit} className="flex gap-3 mb-6 max-w-lg">
        <input type="text" placeholder="Nama" required
          className="flex-1 rounded-lg border border-gold/20 bg-cream px-3 py-2 text-sm text-green-dark placeholder-green-dark/80 focus:outline-none focus:ring-2 focus:ring-gold/50"
        ...
        />
        <button type="submit"
          className="h-10 px-4 rounded-lg bg-gold text-green-dark text-sm font-medium hover:bg-gold-dark"
        >
          ...
          <tbody>
            {categories.map((cat) => (
              <tr key={cat.id} className="border-b border-gold/10 hover:bg-gold/5">
                ...
                <td className="px-4 py-3 text-green-dark/80">{cat.slug}</td>
                <td className="px-4 py-3 text-center space-x-2">
                  <button onClick={() => handleEdit(cat)} className="text-green hover:underline text-xs">Edit</button>

...
export default function MegaMenu({ items, isOpen, onKeepOpen, onStartClose }: Props) {
  ...
  return (
    ...
    >
      <div className="bg-cream/95 backdrop-blur-2xl border border-gold/20 rounded-2xl shadow-2xl p-5">
          ...
          {items.map((item) => {
            ...
            return (
              <Link
                key={item.href}
                href={item.href}
                className="group flex items-start gap-4 p-3.5 rounded-xl hover:bg-gold/10 transition-all duration-250"
              >
                <div className="w-10 h-10 rounded-xl bg-gold/10 flex items-center justify-center shrink-0 group-hover:bg-gold/20 transition-colors duration-250">

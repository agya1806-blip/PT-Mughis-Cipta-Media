...
export function QuotaSection() {
  const { quotaTotal, quotaRegistered, status, isOpen, loading } = useCampaignStatus()
  ...
  return (
    <section className="relative py-24 sm:py-32 bg-gradient-to-br from-green via-green-dark to-green overflow-hidden">
      ...
      <div className="max-w-2xl mx-auto px-6 sm:px-8 lg:px-10 relative">
        <motion.div variants={MOTION.fadeUp} initial="hidden" whileInView="visible" viewport={{ once: true }} className="text-center mb-10">
          ...
          <p className="mt-3 text-cream/60">
            {!loading && mc?.badge === "Telah Ditutup"
              ...
              ? "Program akan segera dibuka. Pantau terus informasi selanjutnya."
              : `Hanya ${quotaTotal} peserta pertama yang bisa mengikuti program ini. Segera daftarkan diri Anda.`}
          </p>
        </motion.div>
        ...
        >
          <ProgressBar total={quotaTotal} current={quotaRegistered} />
        </motion.div>
      </div>
    </section>
  )

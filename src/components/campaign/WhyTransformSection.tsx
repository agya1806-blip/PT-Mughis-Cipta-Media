...
export function WhyTransformSection() {
  return (
    <section className="relative py-24 sm:py-32 bg-gradient-to-br from-green via-green-dark to-green overflow-hidden">
      <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_top_left,rgba(211,194,151,0.10),transparent_50%)]" />
      <div className="max-w-7xl mx-auto px-6 sm:px-8 lg:px-10 relative">
        <motion.div variants={MOTION.fadeUp} initial="hidden" whileInView="visible" viewport={{ once: true }} className="text-center mb-16">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-gold/10 border border-gold/20 mb-6">
            <span className="text-green-dark text-[11px] font-medium uppercase tracking-[0.1em]">Mengapa Bertransformasi</span>
          </div>
          ...
          {whyCards.map((card) => {
            ...
            return (
              <motion.div
                ...
                className="group relative bg-cream/5 backdrop-blur-sm rounded-2xl border border-cream/10 p-6 sm:p-8 hover:bg-cream/10 hover:border-gold/30 transition-all duration-500"
              >
                ...
                <p className="text-sm text-cream/80 leading-relaxed">{card.description}</p>
              </motion.div>
            )
          })}
        </motion.div>

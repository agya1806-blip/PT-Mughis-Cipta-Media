...
export default function CartPage() {
  ...
  if (items.length === 0) {
    return (
      ...
      >
        <div className="mb-4"><ShoppingCart className="w-16 h-16 text-zinc-300" /></div>
  ...
  return (
    ...
    >
      ...
      <motion.div variants={containerVariants} initial="hidden" animate="visible" className="space-y-4">
        {items.map((item) => (
          <motion.div key={item.bookId} variants={itemVariants} className="flex gap-4 bg-white rounded-xl border border-zinc-200 p-4">
      ...
      <motion.div
        ...
        initial="hidden"
        animate="visible"
        className="mt-8 bg-white rounded-xl border border-zinc-200 p-6"
      >
        <div className="flex justify-between text-lg font-semibold text-zinc-800">
          <span>Total ({itemCount} item)</span>

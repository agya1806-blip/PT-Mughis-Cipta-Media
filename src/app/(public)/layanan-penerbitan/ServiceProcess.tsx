      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {process.map((step, i) => (
          <div key={step.label} className="relative flex flex-col items-center text-center p-5 bg-cream rounded-xl border border-gold/20 shadow-sm">
            <div className="w-10 h-10 rounded-full bg-gold text-green-dark flex items-center justify-center font-bold text-sm mb-3">
              {i + 1}
            </div>
            <h4 className="font-semibold text-green-dark text-sm mb-1">{step.label}</h4>
            <p className="text-xs text-green-dark/80">{step.desc}</p>
            {i < process.length - 1 && (

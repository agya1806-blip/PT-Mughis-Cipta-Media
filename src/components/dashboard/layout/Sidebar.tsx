...
export default function Sidebar({ collapsed, onToggle, mobileOpen, onMobileClose }: Props) {
  ...
  const sidebarContent = (
    <div className="flex flex-col h-full">
      <div className="flex items-center gap-3 px-4 h-16 border-b border-gold/20 shrink-0">
        ...
        {!collapsed && (
          <motion.span
            ...
            animate={{ opacity: 1 }}
            className="text-sm font-bold text-green-dark truncate"
          >
            Author Panel
          </motion.span>
        )}
  ...
  return (
    <>
      ...
      />
      <aside
        className={`fixed lg:static inset-y-0 left-0 z-50 flex flex-col bg-cream border-r border-gold/20 transition-all duration-300 ${

...
export function RequirementsSection() {
  return (
    <section className="relative py-24 sm:py-32 bg-gradient-to-br from-green via-green-dark to-green overflow-hidden">
      <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_bottom_right,rgba(211,194,151,0.10),transparent_50%)]" />
      <div className="max-w-2xl mx-auto px-6 sm:px-8 lg:px-10 relative">
        <motion.div variants={MOTION.fadeUp} initial="hidden" whileInView="visible" viewport={{ once: true }} className="text-center mb-12">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-gold/10 border border-gold/20 mb-6">
            <span className="text-green-dark text-[11px] font-medium uppercase tracking-[0.1em]">Persyaratan</span>
          </div>
        ...
        <motion.div variants={MOTION.stagger} initial="hidden" whileInView="visible" viewport={{ once: true }} className="space-y-4">
          {requirements.map((req, i) => (
            <motion.div
              ...
              className="flex items-start gap-4 bg-cream/5 backdrop-blur-sm rounded-xl border border-cream/10 p-5"
            >
